...
export class MemoryStorage implements ItemStorage {
  ...
  async listItems(query: ListItemsQuery): Promise<ListItemsResult> {
    ...
    return {
      items: page,
      total,
      // MemoryStorage uses offset+limit; the token is the next offset
      // encoded as a string so client code can be backend-agnostic.
      ...(hasMore ? { nextToken: String(offset + limit) } : {}),
    };
  }
  ...
  async getAuditTrail(id: string): Promise<ExamItem[]> {
    ...
    return (this.versions.get(id) || []).slice().reverse();
  }

  // Test-only: wipes all in-memory state. Not part of ItemStorage because
  // production backends (DynamoDB) have no equivalent operation.
  reset(): void {
    this.items.clear();
    this.versions.clear();
  }
